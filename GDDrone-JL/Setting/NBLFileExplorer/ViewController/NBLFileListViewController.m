...
@interface NBLFileListViewController ()<UITableViewDataSource, UITableViewDelegate, UIGestureRecognizerDelegate, UISearchBarDelegate, NBLFileMenuToolDelegate, NBLFileGalleryViewDelegate, NBLEditGalleryViewDelegate, UIActionSheetDelegate>{
    int topBarH;
}
...
{
    if (!_searchBGView) {
        UIView *searchBGView = [[UIView alloc] initWithFrame:CGRectMake(0, 0+topBarH, self.view.frame.size.width, 40)];
...
{
    if (!_backButtonItem) {
        UIBarButtonItem *backButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"leftbtnImg"] style:UIBarButtonItemStylePlain target:self action:@selector(backAction)];
...
- (UILabel *)currentLocationLabel
{
    if (!_currentLocationLabel) {
        UILabel *currentLocationLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0+topBarH, 200, 40)];
...
{
    if (!_tableView) {
        UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 10+topBarH, self.view.frame.size.width, self.view.frame.size.height - 10) style:UITableViewStylePlain];
...
{
    if (!_galleryView) {
        NBLFileGalleryView *galleryView = [[NBLFileGalleryView alloc] initWithFrame:CGRectMake(0, 50+topBarH, self.view.frame.size.width, self.view.frame.size.height - 50)];
...
//    CGRect frame = CGRectMake(0, 0, kHeight, 50);
    ...
    if (error) {
        ...
        for (NSString *fileName in files) {
            ...
            if (isFileExist) {
                ...
                if([fileName containsString:@".bfu"]){//[fileName isEqualToString:@"Documents"] ||
                    [filesMutArr addObject:fileInfo];
                }
...
//        [self updateBackButton:[UIImage imageNamed:@"gd_back_0"]];
...
- (void)jumpToNextFolderWithIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (fileInfo.isFolder) {
        ...
        [self updateBackButton:[UIImage imageNamed:@"leftbtnImg"]];
        [self refreshCurrentLocation];
    } else {
        
        //if([fileInfo.fileName isEqualToString:@"JL_AC54.bfu"]){
        if([fileInfo.fileName containsString:@".bfu"]){
            [self dismissViewControllerAnimated:YES completion:nil];
            [[NBLFileMenuTool sharedTool] destoryMenu];
            
            NSString *filePath =fileInfo.filePath;
            [[NSNotificationCenter defaultCenter] postNotificationName:@"CTP_LOCAL" object:filePath];
        }
//        //open file option
