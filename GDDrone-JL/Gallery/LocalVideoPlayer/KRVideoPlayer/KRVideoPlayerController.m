...
- (void)configObserver
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onMPMovieDurationDidPlayOver:) name:MPMoviePlayerPlaybackDidFinishNotification object:nil];
}
...
-(void)onMPMovieDurationDidPlayOver:(NSNotification *)note{
...
{
    double currentTime = floor(self.currentPlaybackTime+1);
    double totalTime = floor(self.duration);
    [self setTimeLabelValues:currentTime totalTime:totalTime];
